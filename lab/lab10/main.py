...
def graph_from_edgelist(E, directed=False):
  ...
  """
  g = Graph(directed)
  # V is a local listing of strings that match the vertices
  V = set()
  for e in E:
    V.add(e[0])
    V.add(e[1])
  ...
  # list the edges for MIA
  print ("\n\nincoming MIA edges\n")
  for e in g.incident_edges(verts['MIA'], False):
    print (e)

  print ("\n\noutgoing MIA edges\n")
  for e in g.incident_edges(verts['MIA'], True):
    print (e)
  
  dfstree = {verts['MIA'] : None}
  DFS(g, verts['MIA'], dfstree)
  print ("\n\nDFS from MIA visited nodes\n")
  for key in dfstree.keys():
    print (key)

  print ("\n\nDFS from MIA to Parkland\n")
  path = construct_path(verts['MIA'], verts['Parkland'], dfstree)
  for stop in path:
    print (stop)


  # bfs 
  bfstree = {verts['BOS'] : None}
  BFS(g, verts['BOS'], bfstree)
  print ("\n\nBFS from BOS visited nodes\n")
  for key in bfstree.keys():
    print (key)

  print ("\n\nBFS from BOS to Parkland\n")
  path = construct_path(verts['BOS'], verts['Parkland'], bfstree)
  for stop in path:
    print (stop)
  

  # shortest paths from ORD to everywhere
  sp_distmap = shortest_path_lengths(g, verts['ORD'])
  sptree = shortest_path_tree(g, verts['ORD'], sp_distmap)
  print ("\n\nShortest Path from ORD to Parkland\n")
  path = construct_path(verts['ORD'], verts['Parkland'], sptree)
  for stop in path:
    print (stop)
